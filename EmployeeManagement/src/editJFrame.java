...
public class editJFrame extends javax.swing.JFrame {
    ...
    //ATTRIBUTES
    public Hashtable editHT;
    public AddJFrame thisAJF;
    public FileManager tFiles;
    public EmployeeInfo theEmployee;
    public FTE theFTE;
    public PTE thePTE;
    ...
    public void setEMP(EmployeeInfo theEmp) {
        jLabel14.setText("Editting Employee NO. " + theEmp.employeeNum);
        theEmployee = theEmp;
        inputEmpNum.setText( Integer.toString(theEmployee.employeeNum) );
        inputEmpNum.setEditable(true);
        inputFirstName.setText(theEmployee.firstName);
        inputLastName.setText(theEmployee.lastName);
        setEmpNum.setSelected(true);
        inputWorkLoc.setText(theEmployee.workLoc);
        if (theEmployee.gender == 0) {
            femaleGender.setSelected(false);
            otherGender.setSelected(false);
            maleGender.setSelected(true);
        }
        if (theEmployee.gender == 1) {
            maleGender.setSelected(false);
            otherGender.setSelected(false);
            femaleGender.setSelected(true);
        }
        if (theEmployee.gender == 2) {
            femaleGender.setSelected(false);
            maleGender.setSelected(false);
            otherGender.setSelected(true);
        }
        
        if (theEmployee.deductRate != 0.21) {
            useCustom.setSelected(true);
            deductField.setText( Double.toString(theEmployee.deductRate) );
        }
        
        if (theEmployee instanceof FTE) {
            theFTE = (FTE) theEmployee;
            annualSalaryField.setText( Double.toString(theFTE.yearlySalary) );
        ...
        if (theEmployee instanceof PTE) {
            thePTE = (PTE) theEmployee;
            hoursField.setText( Double.toString(thePTE.hourlyWage) );
            weeksField.setText( Double.toString(thePTE.hoursPerWeek) );
            yearsField.setText( Double.toString(thePTE.weeksPerYear) );
            partTimeStatus.setSelected(true);
    ...
    private void finishAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_finishAddActionPerformed
        // TODO add your handling code here:
        editHT.remove(theEmployee.employeeNum);
        ...
        if (fullTimeStatus.isSelected() == true) {
            ...
            if (annualSalary == annualSalary) {
                ...
                if (annualSalary.length() > 9) {
                    System.out.println("Employee Number is too long.");
                    return;

                }

                for (int b = 0; b < annualSalary.length(); b++) {

                    if (Character.isLetter(annualSalary.charAt(b)) == true) {
        ...
        if (partTimeStatus.isSelected() == true) {
            ...
            if (hoursWage == hoursWage) {
                String checkSal = null;
                if (hoursWage.length() <= 0) {
                    System.out.println("Nothing in hourly wage.");
                    return;
                }
                if (hoursWage.length() > 9) {
                    System.out.println("Hourly wage is too long.");
                    return;

                }

                for (int b = 0; b < hoursWage.length(); b++) {

                    if (Character.isLetter(hoursWage.charAt(b)) == true) {
                        System.out.println("Not a valid number.");
                        checkSal = "flag";
                        return;
                    }
